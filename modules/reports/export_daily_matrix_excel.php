<?php
// modules/reports/export_daily_matrix_excel.php
...
header("Content-Disposition: attachment; filename=\"Laporan_Mutasi_Telur_Matrix_{$filter_whid}_" . date('Ymd') . ".xls\"");
...
<head>
    ...
    <style>
        ...
        th, td { border: 1px solid #000000; padding: 4px; text-align: right; }
...
<body>
    <h3>Laporan Mutasi Telur Matrix (Gudang: <?= htmlspecialchars($filter_whid) ?>)</h3>
    <p>Periode: <?= htmlspecialchars($filter_from) ?> s/d <?= htmlspecialchars($filter_to) ?></p>
    ...
    <table>
        ...
        <tbody>
            <?php foreach ($daily_data as $row): ?>
                <tr>
                    <td class="text-center text-bold"><?= $row['dayName'] ?> <?= $row['formattedDate'] ?></td>
